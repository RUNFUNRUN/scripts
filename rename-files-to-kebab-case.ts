...
const renameFiles = async(directory: string): Promise<void> => {
  for await (const entry of walk(directory, { exts: ["ts", "tsx"], includeDirs: false })) {
...
if (import.meta.main) {
  ...
  if (!directory) {
    console.error("Usage: deno run --allow-read --allow-write rename-files-to-kebab-case.ts <directory>");
    Deno.exit(1);
  }
